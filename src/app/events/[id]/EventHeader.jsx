  ...
  return (
    <div className="relative w-full">
      ...
      <div className="relative w-full h-[220px] sm:h-[280px] md:h-[320px] lg:h-[380px] overflow-hidden rounded-b-3xl">
        ...
        {event?.videoUrl ? (
          <iframe
            src={`${event.videoUrl}?autoplay=true&muted=true&loop=true&controls=false`}
        ...
        {event?.trailerUrl && (
          ...
          >
            <div className="bg-white/20 backdrop-blur-md p-5 rounded-full hover:scale-110 transition border border-white/30">
      ...
      <div className="absolute top-4 left-4 z-30">
        <Link
          href="/events"
          className="flex items-center gap-2 bg-black/40 backdrop-blur-sm px-3 py-2 rounded-full text-sm text-white hover:bg-black/60 transition"
        >
          <ArrowLeft size={18} />
      ...
      <div className="absolute top-4 right-4 z-30">
        <button
          ...
          className="bg-black/40 cursor-pointer backdrop-blur-sm p-2 rounded-full text-white hover:bg-black/60 transition"
        >
          <Share2 size={18} />
        </button>
      </div>
      ...
      {/* Poster Card */}
      <div className="absolute -bottom-16 left-4 sm:left-6 w-28 sm:w-32 md:w-36 shadow-2xl z-20">
          ...
          {event?.status && (
            <span
              className={`absolute top-2 left-2 ${getStatusColor(
                event.status,
              )} text-[10px] sm:text-xs font-semibold px-2 py-1 rounded-full capitalize`}
            >
              {event.status}
            </span>
          )}
